...
# All input arrays will be valid integer arrays (although it could still be empty), so you 
# won't need to validate the input.
  ...
  for i in range(1, len(arr)-1):
    ...
    if arr[i] > arr[i-1] and arr[i] == arr[i+1]:
      if arr[i+2] < arr[i]:
        peaks.append(arr[i])
        pos.append(i)
      elif any(y < arr[i] for y in arr[i:]) and not any(y > arr[i] for y in arr[i:]):
        peaks.append(arr[i])
        pos.append(i)
